...
server.ts's `hypothesis_*` verbs call after every successful mutation (`server.ts:853#"saveStore(sessionId, store);"`,
`server.ts:899#"saveStore(sessionId, store);"`, `server.ts:931#"saveStore(sessionId, store);"`,
`server.ts:953#"saveStore(sessionId, store);"`, `server.ts:980#"saveStore(sessionId, store);"`).
...
(`packages/grounding-mcp/README.md:224#"the root cause is the backend container's missing OPENAI_API_KEY env var"`).
...
`loadStoreFromDisk` at `hypothesis-store.ts:132#"function loadStoreFromDisk"`), at the cost of one extra disk read, not the hypotheses
themselves.
...
- `hypothesis_record` (`server.ts:839#"'hypothesis_record',"`): add a competing hypothesis with required checks
- `hypothesis_list` (`server.ts:859#"'hypothesis_list',"`): all hypotheses for a session + status summary
- `hypothesis_evidence` (`server.ts:882#"'hypothesis_evidence',"`): attach evidence; **auto-promotes** (the `addEvidence` path above)
- `hypothesis_check_done` (`server.ts:905#"'hypothesis_check_done',"`): mark a `required_checks[i]` done; drains `pending_checks`
- `hypothesis_reject` (`server.ts:937#"'hypothesis_reject',"`): reject with reason (appended as `[rejected]` evidence)
- `hypothesis_support` (`server.ts:959#"'hypothesis_support',"`): explicit support; the checks-gated path; error
  `hypothesis_not_found_rejected_or_checks_pending` when it returns null (`server.ts:975#"error: 'hypothesis_not_found_rejected_or_checks_pending',"`)
- `hypothesis_reset` (`server.ts:986#"'hypothesis_reset',"`): purge a session's hypotheses (MCP counterpart of `resetStore`), also deletes the on-disk file (`hypothesis-store.ts:257-261#"return existedInMemory || existedOnDisk;"`)
