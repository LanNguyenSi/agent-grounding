...
server.ts's `hypothesis_*` verbs call after every successful mutation (`server.ts:828#"saveStore(sessionId, store);"`,
`server.ts:874#"saveStore(sessionId, store);"`, `server.ts:906#"saveStore(sessionId, store);"`,
`server.ts:928#"saveStore(sessionId, store);"`, `server.ts:955#"saveStore(sessionId, store);"`).
...
- `hypothesis_record` (`server.ts:814#"'hypothesis_record',"`): add a competing hypothesis with required checks
- `hypothesis_list` (`server.ts:834#"'hypothesis_list',"`): all hypotheses for a session + status summary
- `hypothesis_evidence` (`server.ts:857#"'hypothesis_evidence',"`): attach evidence; **auto-promotes** (the `addEvidence` path above)
- `hypothesis_check_done` (`server.ts:880#"'hypothesis_check_done',"`): mark a `required_checks[i]` done; drains `pending_checks`
- `hypothesis_reject` (`server.ts:912#"'hypothesis_reject',"`): reject with reason (appended as `[rejected]` evidence)
- `hypothesis_support` (`server.ts:934#"'hypothesis_support',"`): explicit support; the checks-gated path; error
  `hypothesis_not_found_rejected_or_checks_pending` when it returns null (`server.ts:950#"error: 'hypothesis_not_found_rejected_or_checks_pending',"`)
- `hypothesis_reset` (`server.ts:961#"'hypothesis_reset',"`): purge a session's hypotheses (MCP counterpart of `resetStore`), also deletes the on-disk file (`hypothesis-store.ts:257-261#"return existedInMemory || existedOnDisk;"`)
