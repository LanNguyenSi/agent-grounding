...
timestamp: 2026-09-23T08:18:16Z
sources:
...
- **`solution_evaluate`** (registered `server.ts:694#"'solution_evaluate'"`): the producer. Runs preflight against
...
- **`solution_gate`** (registered `server.ts:769#"'solution_gate'"`): read-only checker. Resolves current HEAD
