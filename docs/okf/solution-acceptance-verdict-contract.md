...
timestamp: 2026-09-23T09:44:31Z
sources:
...
- **`solution_evaluate`** (registered `server.ts:719#"'solution_evaluate'"`): the producer. Runs preflight against
...
- **`solution_gate`** (registered `server.ts:794#"'solution_gate'"`): read-only checker. Resolves current HEAD
