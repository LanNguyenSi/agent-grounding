...
- **The merge-approval CI Action keys by the PR head branch name.** `.github/workflows/merge-approval.yml:218#"task-id: ${{ github.event.pull_request.head.ref }}"` passes `task-id: ${{ github.event.pull_request.head.ref }}` into the `review-claim-gate/action`. `head.ref` is the **PR HEAD BRANCH NAME**, not a task UUID. This correction is load-bearing: any mental model that assumes CI reads evidence under a task UUID is wrong: it reads under the literal branch string.
...
- **CI reader convention (branch name):** `.github/workflows/merge-approval.yml:218#"task-id: ${{ github.event.pull_request.head.ref }}"` (`task-id: …head.ref`), consumed by the action pinned at `.github/workflows/merge-approval.yml:216#"review-claim-gate-v0.1.6"`.
