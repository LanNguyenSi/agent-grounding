...
- **Writer convention (`gs-*`):** `packages/grounding-mcp/src/server.ts:557#"Session id: used as the ledger session namespace."` (param doc), `packages/grounding-mcp/src/server.ts:565-572#"session: sessionId,"` (write-through); id shape `packages/grounding-wrapper/src/lib.ts:58-61#"gs-${slug}-${ts}"`.
