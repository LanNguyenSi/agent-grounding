...
timestamp: 2026-09-06T05:39:55Z
sources:
...
- **Writer convention (`gs-*`):** `packages/grounding-mcp/src/server.ts:232#"Session id — used as the ledger session namespace."` (param doc), `packages/grounding-mcp/src/server.ts:239-245#"session: sessionId,"` (write-through); id shape `packages/grounding-wrapper/src/lib.ts:58-61#"gs-${slug}-${ts}"`.
