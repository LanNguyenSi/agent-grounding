...
- **Writer convention (`gs-*`):** `packages/grounding-mcp/src/server.ts:581#"Session id: used as the ledger session namespace."` (param doc), `packages/grounding-mcp/src/server.ts:589-595#"session: sessionId,"` (write-through); id shape `packages/grounding-wrapper/src/lib.ts:58-61#"gs-${slug}-${ts}"`.
