...
test('run(): M1 negative control -- a ci.yml copy pinning okf-kit@0.10.1 (fixtures unchanged, still 0.10.0) exits 1 on the version mismatch', () => {
  const before = fs.readFileSync(REAL_CI_YAML, 'utf8');
  const mutated = before.replace('npm install -g okf-kit@0.10.0', 'npm install -g okf-kit@0.10.1');
