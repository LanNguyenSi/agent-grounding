...
// Shim re-exports the plugin function from the npm-installed package.
// opencode auto-loads any *.ts/*.js file under .opencode/plugins/, so no
// explicit registration in opencode.json is required.
//
// The user still needs the @lannguyensi/understanding-gate package
// available to opencode's Bun runtime. The init command prints the
// install steps; if the package is not on the public registry yet, a
// local-path install (`npm i -D /path/to/agent-grounding/packages/understanding-gate`)
// or a `.opencode/package.json` entry pointing to a tarball works too.
...
// installed @lannguyensi/understanding-gate package. opencode auto-loads
// every file under .opencode/plugins/, so no opencode.json change is needed.
