...
| `ledger_summary` | `evidence-ledger.getSummary` | Return all entries for a session, grouped by type, with counts. A zero count is not necessarily an error: causes include (not an exhaustive list) no entries yet, a sessionId that does not exactly match what `ledger_add` used, or a `contentPrefix` filter that excludes every matching row. `sinceIso` must be an ISO-8601 date (`"2026-05-01"`) or a datetime with an explicit `Z` or numeric offset (`"2026-05-01T08:00:00Z"`, `"2026-05-01T10:00:00+02:00"`); an empty string, a relative shorthand (`"1h"`, `"24h"`, `"yesterday"`), an epoch number, `Date.toString()` output, or a local datetime without a zone is rejected with a validation error naming the field, not silently treated as "matches nothing" (SQLite's `datetime()` would otherwise return `NULL` for those and exclude every row). Ordered by arrival at the transport, see `ledger_add`. |
