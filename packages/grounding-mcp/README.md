...
Round 3 (review findings F1/F2) tightened the prose fallback after a corpus replay found it fail-open in two ways. **F1:** a recognized value token only counts when the text after it is empty, end-of-sentence punctuation only, or an opening `(` (the template's own filled shape, `Method: adversarial (briefing and return match).`); a real run's line `Method: rigorous for every round except T-007 R1 and T-010 R1 (adversarial); ...` has `rigorous` as its first token but is a qualified, multi-round sentence, not a record of the immediately-preceding round, so it is now a malformed record rather than silently read as `rigorous`. **F2:** the prose fallback is only read when the declaration's OWN line carries exactly one well-formed `review-method[...]` occurrence; several rounds packed onto one shared line followed by one `Method:` summary line (real shape: `quickwins-batch48`'s `T-011-R1`/`R2`/`R3`) no longer let that single line clear all of them: such rounds fall through to the "no matching record" (absent) reason instead, the same outcome as if no `Method:` line followed at all. Two accepted residuals: a fenced code block between the declaration and its `Method:` line is blanked by the quoting strip and so is skipped over rather than treated as intervening content that would break the "next line" adjacency; and only `05-review-findings.md` is read at all, so a `method-applied[<round>]` recorded in `03-decisions.md` or `04-implementation-summary.md` instead is invisible to this check (the criterion this reader enforces allows either channel, but only this one file is consulted).
