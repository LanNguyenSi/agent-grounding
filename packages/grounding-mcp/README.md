...
| `ledger_add` | `evidence-ledger.addEntry` | Append a fact / hypothesis / rejected / unknown to the session's ledger namespace. Only a `ledger_summary` call using this exact sessionId string sees it back (case-sensitive, no normalization). |
| `ledger_summary` | `evidence-ledger.getSummary` | Return all entries for a session, grouped by type, with counts. Zero counts (not an error) mean either no entries yet, or a sessionId that does not exactly match what `ledger_add` used. |
