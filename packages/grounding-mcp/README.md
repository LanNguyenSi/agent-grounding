...
**Ids the lookups cannot use.** Both lookups bound `id` at 200 characters. The bound is theirs alone: `solution_evaluate` keeps its unbounded schema, because an id too long for the filesystem already comes back from it as an ordinary `{status:"failed", error}` payload, and 200 is what a verdict id has to fit into, since every id becomes a file name (marker, attempt log, lock anchor, compaction temp file) inside the verdict dir, and the longest of those names runs about 45 characters past the id itself. An id that passes the bound but is still unusable, `..` for instance, comes back as `{"status":"unknown","id":"..","error":"..."}` rather than as a tool error: `unknown` is the honest answer (no attempt could be identified, and none was recorded), and it matches what `solution_evaluate` already does with the same id. An id is never a path: `sanitizeVerdictId` reduces it to one safe segment first, so `../../etc/passwd` is simply an id that nothing ever ran.
