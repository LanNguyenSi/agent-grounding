...
// has a missing, empty, or non-string "version" field, instead of silently
// falling back to '0.0.0'. A mutant that drops the stderr write (in the
// catch block or the missing-version branch) must fail these tests. Also
// asserts the diagnostic write itself never throws (a throwing
// process.stderr.write, a throwing Error#message getter, or a throwing
// String(err) on a non-Error value) and that a multi-line error message is
// collapsed to a single stderr line.
