// Regression guard for the published `claim-gate` bin entrypoint.
//
...
describe("claim-gate bin entrypoint via symlink", () => {
  it.skipIf(!existsSync(distCli))(
    "runs through a node_modules/.bin-style symlink (not a silent no-op)",
    () => {
      const dir = mkdtempSync(join(tmpdir(), "claim-gate-bin-"));
      const link = join(dir, "claim-gate");
      try {
        symlinkSync(distCli, link);
        // --help short-circuits in commander, exits 0.
