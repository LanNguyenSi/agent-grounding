...
7. **Push the release branch and open a PR** with a Markdown test-plan checklist in the body, naming each dogfood case and its expected vs observed outcome. The PR body is a release note: it ends up referenced from the CHANGELOG and the maintainer reads it before tagging.
   ...
   **PR creation on this repo needs a human-owned credential, not the App token.** The GitHub App installation token this repo's agents mint via `gh-token.sh` cannot create pull requests here (`createPullRequest` is not accessible by the app's own integration/installation identity on this repo). Use the agent-tasks `pull_requests_create` MCP tool, or `gh pr create` under the operator's own/OPS `gh` auth, instead. This is what the 2026-09-06 release batch used; if you hit a PR-creation 403/permission error while cutting a release here, this is why — do not spend time debugging the App token first.
